...
\subsection{Prosta - równanie kierunkowe}
$$f(x) = ax + b;\;\; a, b \in \mathbb{R}$$
\noindent Prosta przecina oś OY w punkcie $(0, b)$, a oś OX w $(-\frac{b}{a}, 0)$. Prosta
jest nachylona do osi OX pod kątem $\alpha$ takim, że $\text{tg}\;\alpha = a$.\\

\noindent Jeżeli podane są dwa punkty $A(x_{A}, y_{A}), B(x_{B}, y_{B})$, przez które przechodzi prosta, to równanie kierunkowe prostej
można wyznaczyć ze wzoru:
$$(y - y_{A})(y_{B}-y_{A}) = (x_{B}-x_{A})(x - x_{A})$$
Współczynnik kierunkowy prostej przechodzącej przez punkty: $A(x_{A}, y_{A}), B(x_{B}, y_{B})$ opisuje
wzór:
$$a = \dfrac{y_{B}-y_{A}}{x_{B}-x_{A}}$$
Równanie prostej o współczynniku kierunkowym $a$, przechodzącej przez punkt $A(x_{A}, y_{A})$ podane
jest wzorem:
$$y = a(x - x_{A}) + y_{A}$$
